...
function runnerSsh (task, worker, _log) {
  return new PCancelable((onCancel, resolve, reject) => {
    ...
    if (worker.server.identity) {
      worker.server.identity.forEach(identity => cmdOptions.push('-i', identity))
    }
    ...
    log({ type: 'exec', cmd: `ssh ${cmdOptions.join(' ')}` })

    let run = runnerSsh.execa('ssh', cmdOptions, { input })
